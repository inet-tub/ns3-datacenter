...
for BURST_SIZES in ${BURSTS[@]};do
	for SWITCH in 0 1 2 3 4 5;do
		...
		DUMPMODELFILE="$DUMP_DIR/model-$TCP-$LOAD-$BURST_SIZES-$BURST_FREQ-$CDFNAME-$SWITCH.joblib"
		DUMPSCOREFILE="$DUMP_DIR/model-$TCP-$LOAD-$BURST_SIZES-$BURST_FREQ-$CDFNAME-$SWITCH.score"
		N=$(( $N+1 ))
		$PYTHON trainLqd.py $LQDOUTFILE $DUMPMODELFILE $MAX_DEPTH $N_ESTIMATORS > $DUMPSCOREFILE 2> /dev/null
