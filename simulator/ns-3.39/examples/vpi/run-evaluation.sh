	...
	for ROUTING in $RANDOM_ECMP;do
		...
		for QP_WINDOW in 256;do
			...
			for QP_RANDOMIZE in "true";do
				...
				FCTFILE=$DUMP_DIR/evaluation-$RDMACC-$MULTI_PATH-$ROUTING-$QP_RANDOMIZE-$QP_WINDOW-$TRANSFER_SIZE-$COLLECTIVE.fct
				TORFILE=$DUMP_DIR/evaluation-$RDMACC-$MULTI_PATH-$ROUTING-$QP_RANDOMIZE-$QP_WINDOW-$TRANSFER_SIZE-$COLLECTIVE.tor
				DUMPFILE=$DUMP_DIR/evaluation-$RDMACC-$MULTI_PATH-$ROUTING-$QP_RANDOMIZE-$QP_WINDOW-$TRANSFER_SIZE-$COLLECTIVE.out
				PFCFILE=$DUMP_DIR/evaluation-$RDMACC-$MULTI_PATH-$ROUTING-$QP_RANDOMIZE-$QP_WINDOW-$TRANSFER_SIZE-$COLLECTIVE.pfc
				echo $FCTFILE
				if [[ $EXP == 1 ]];then
