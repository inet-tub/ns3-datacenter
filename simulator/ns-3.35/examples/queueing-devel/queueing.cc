...
bool enableFctLog=false;

Ptr<OutputStreamWrapper> torStats;
AsciiTraceHelper torTraceHelper;
bool enableTorLog = false;

Ptr<OutputStreamWrapper> rxOutput;
AsciiTraceHelper rxTraceHelper;

Ptr<OutputStreamWrapper> txOutput;
AsciiTraceHelper txTraceHelper;
...
void
TraceTxQueueDisc(Ptr<OutputStreamWrapper> stream, Ptr<Packet const> p, uint32_t qIndex, Ptr<GenQueueDisc> genDisc) {
	UnSchedTag tag; uint32_t unsched = 0;
	bool found = p->PeekPacketTag(tag);
	if (found) unsched = tag.GetValue();
	*stream->GetStream()
...
main (int argc, char *argv[])
{
	...
	std::string rxOutFile = "./arrival.txt";
	cmd.AddValue ("rxOutFile", "File path for tracing packet rx events at switch queue discs", rxOutFile);

	std::string txOutFile = "./departure.txt";
	cmd.AddValue ("txOutFile", "File path for tracing packet tx events at switch queue discs", txOutFile);
	...
	for (uint32_t leaf = 0; leaf < LEAF_COUNT; leaf++) {
		...
		for (uint32_t server = 0; server < SERVER_COUNT; server++) {
			...
			genDisc->setNodeId(leaf);
			genDisc->setNodeType("ToRServer");
			if (leaf == 1 && server == 4){ // leaf x and server y just some randomly chosen downlink to log values
				if (rxOutFile!="") genDisc->TraceConnectWithoutContext("genEnqueue", MakeBoundCallback(&TraceRxQueueDisc, rxOutput));
				if (txOutFile!="") genDisc->TraceConnectWithoutContext("genDequeue", MakeBoundCallback(&TraceTxQueueDisc, txOutput));
			}
			switch (algorithm) {
			case DT:
	...
	for (uint32_t leaf = 0; leaf < LEAF_COUNT; leaf++) {
		for (uint32_t spine = 0; spine < SPINE_COUNT; spine++) {
			for (uint32_t link = 0; link < LINK_COUNT; link++) {
				...
				genDisc[0]->setNodeId(leaf);
				genDisc[0]->setNodeType("ToRSpine");
				genDisc[1]->setNodeId(spine);
				genDisc[1]->setNodeType("SpineToR");
				// genDisc[0]->TraceConnectWithoutContext("genEnqueue", MakeBoundCallback(&TraceRxQueueDisc, rxOutput));
