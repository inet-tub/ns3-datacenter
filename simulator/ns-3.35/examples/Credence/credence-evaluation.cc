...
void TraceLQD(Ptr<OutputStreamWrapper> stream, Ptr<const Packet> p, uint32_t drop){
	BufferLogTag buffertag;
	bool found = p->PeekPacketTag(buffertag);
	if (found){
		*stream->GetStream()
			<< buffertag.getQueueLength() << " "
			<< buffertag.getAverageQueueLength() << " "
			<< buffertag.getOccupiedBuffer() << " "
			<< buffertag.getAverageOccupiedBuffer() << " "
			<< drop << std::endl;
	}
}
...
main (int argc, char *argv[])
{
	...
	//py::print("HELLLLOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO");
	...
	uint32_t BufferSize = (5.12*1000)*(SERVER_COUNT+SPINE_COUNT*LINK_COUNT)*(spineLeafCapacity);
	...
	uint32_t requestSize = 4 * BufferSize;
	double queryRequestRate = 1; // at each server (per second)
	...
	std::string lqdOutFile = "./examples/Credence/lqd-logs/WS-";
