...
- [`simulator/ns-3.35/src/point-to-point/switch-node.cc`](https://github.com/inet-tub/ns3-datacenter/tree/master/simulator/ns-3.35/src/point-to-point/switch-node.cc): This file is also modified to support both RDMA and TCP/IP traffic. Minor change in INT, for PowerTCP, RXBytes is appended instead of TxBytes. This is since RxRate is TxRate + Queue gradient where $\lambda$ i.e RxRate is required for PowerTCP to calculate power.
